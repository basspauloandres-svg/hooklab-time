...
# Controlled M-only execution from the verified full master.
# H and T are never recomputed by this script.
# Optional: set DEMUCS_REPO to a local directory containing htdemucs.yaml
# and 955717e8-8726e21a.th to run without network access.
...
python -m demucs "${DEMUCS_ARGS[@]}" "$OUT/input/golden_source_window.wav"
VOCAL=$(find "$OUT/stems" -type f -name vocals.wav -print -quit)
if [[ -z "$VOCAL" ]]; then
...
echo "M v0.7 source-window probe completed. H/T remain frozen; no audible substitution rendered."
