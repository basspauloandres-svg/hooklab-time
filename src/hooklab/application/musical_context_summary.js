...
export function summarizeMusicalContext({aligned, behavioralTimestampMs, lookbackMs=5000, lookaheadMs=2000}){
  ...
  const candidates=(aligned??[])
    .map(item=>item?.stimulusEvent??item?.stimulus??item)
    .filter(Boolean)
    .map(event=>({event,timestampMs:eventTime(event)}))
    .filter(x=>x.timestampMs!=null&&x.timestampMs>=behavioralTimestampMs-lookbackMs&&x.timestampMs<=behavioralTimestampMs+lookaheadMs)
  ...
  return Object.freeze({
    ...
    window:Object.freeze({startMs:behavioralTimestampMs-lookbackMs,endMs:behavioralTimestampMs+lookaheadMs}),
