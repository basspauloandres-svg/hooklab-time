...
def analyze(raw,target,member):
    ...
    base={'title':target['title'],'artist':target['artist'],'year':target.get('year'),'md5':target['md5'],'archive_member':member,'duration_seconds':dur,'lyric_event_count':len(le),'text_token_count':len(tokens),'coverage':'FULL_SONG' if dur>=90 else 'INSUFFICIENT','evidence_role':'TARGET_CANDIDATE_PENDING_IDENTITY_AUDIT'}
    if not cand:return base|{'structural_status':'NO_MELODY_CANDIDATE','full_tmt_candidate':False}
    score,inst,notes,pitches,ov,ept,cov,pr,med=cand;tempi=pm.get_tempo_changes()[1];density_ok=.35<=ept<=2.;melody_ok=score>=7 and ov<=.25 and cov>=.55 and 5<=pr<=36 and 48<=med<=84 and density_ok;text_ok=len(tokens)>=20 and len(le)>=20;tmt=dur>=90 and melody_ok and text_ok
    return base|{'structural_status':'FULLSONG_SYMBOLIC_AUDIT_PASS' if dur>=90 and melody_ok else 'SYMBOLIC_AUDIT_REQUIRED','full_tmt_candidate':tmt,'tempo_bpm':float(statistics.median(tempi)) if len(tempi) else None,'melodic_register_midi':med,'melodic_range_semitones':pr,'melodic_events_per_token':ept,'near_tactus_share':near_tactus(pm,notes),'text_line_count':1+sum(1 for a,b in zip(le,le[1:]) if b[0]-a[0]>=1.4) if le else 0,'melody_event_count':len(notes),'melody_track_name':inst.name,'melody_program':int(inst.program),'melody_overlap_ratio':ov,'melody_track_coverage':cov,'melody_candidate_score':score,'melody_density_gate':density_ok,'text_alignment':'MIDI_META_EVENT_TIMED' if le else 'NONE','selection_semantics':'TEXT_CONDITIONED_STRUCTURAL_HEURISTIC_NOT_VOCAL_IDENTITY_PROOF'}
...
def main():
    ...
    if remaining:
        archive_scanned=True;req=urllib.request.Request(a.archive_url,headers={'User-Agent':'HookLabPrototype/1.2'})
    ...
    summary={'schema':'HOOKLAB_LMD_FULL_MD5_TARGET_AUDIT_v1.2','archive':a.archive_url,'targets':len(by),'found':len(rows),'missing_md5':sorted(remaining),'full_tmt_candidates':sum(bool(r.get('full_tmt_candidate')) for r in rows),'archive_scanned_this_run':archive_scanned,'members_scanned_this_run':members,'cache_dir':str(cache),'promotion_rule':'No row becomes massive-hit evidence until identity/version, melodic-reference, genre/style and success-evidence gates pass.'}
