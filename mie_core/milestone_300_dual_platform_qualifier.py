...
def qualify(frame, yt_rows):
 ...
 for x in frame['candidates']:
  ...
  cand.sort(key=lambda z:(z[0],z[1],float(z[2].get('Views') or 0)),reverse=True)
  ...
  if best:
   ...
   youtube=views>=THRESHOLD
   identity='PASS' if best[0]>=.94 and best[1]>=.5 else 'AUDIT'
   dual='PASS' if spotify and youtube and identity=='PASS' else ('FAIL' if (not spotify or not youtube) and identity=='PASS' else 'AUDIT')
   ...
   out.append({**x,'youtube_views_observed':None,'identity':'AUDIT','spotify_success':'PASS' if spotify else 'FAIL','youtube_success':'AUDIT','mass_success_dual_platform':'AUDIT'})
