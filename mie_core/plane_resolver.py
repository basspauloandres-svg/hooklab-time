#!/usr/bin/env python3
"""MIE Competitive Plane Resolver reconstruction v0.3.
...
def resolve_planes(events,vocal_path,config=None):
    ...
    if not src:
        return {'version':'MIE Competitive Plane Resolver reconstruction v0.3','events':[],
    ...
    for seg_i,seg in enumerate(_segments(src,cfg)):
        ...
        for e in seg:
            ...
            states.append([(m,sh,(v/max(vmax,1e-12))) for (m,sh),v in zip(cand,vals)])
        if not states or any(not s for s in states):
            continue
        prev=[]; back=[]
        for m,sh,ac in states[0]:
        ...
        for i in range(1,len(seg)):
            ...
            for j,(m,sh,ac) in enumerate(states[i]):
                best=-1e99; arg=-1
                acoustic=cfg['acoustic_weight']*math.log(0.03+ac)
                for k,(pm,psh,pac) in enumerate(states[i-1]):
    ...
    return {
        'version':'MIE Competitive Plane Resolver reconstruction v0.3',
