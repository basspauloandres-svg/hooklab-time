...
def resolve_preview(track,artist):
 ...
 req=urllib.request.Request(url,headers={'User-Agent':'HookLabPrototype/1.0'})
 with urllib.request.urlopen(req,timeout=30) as r: data=json.load(r)
 nt,na=norm(track),norm(artist)
 scored=[]
...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--output',required=True); ap.add_argument('--limit',type=int,default=3); a=ap.parse_args()
 ...
 for c in DEFAULT_CANDIDATES[:a.limit]:
  ...
  prov={"schema":"HOOKLAB_FREE_PREVIEW_PROVENANCE_v1","candidate":c,"itunes_query":query_url,
  ...
  summary.append({"candidate":c,"resolved_track_id":hit.get('trackId'),"metrics":metrics})
 (root/'batch_summary.json').write_text(json.dumps({"schema":"HOOKLAB_AUTONOMOUS_PUBLIC_BATCH_v1","results":summary},indent=2,ensure_ascii=False))
