...
genre/style, YouTube reach and Spotify reach gates pass. TikTok social reach is computed
when independently observed; missing TikTok evidence remains unresolved. Melodic-reference
validation remains an explicit pending gate before scientific promotion.
"""
...
def main():
 ...
 audit=list(csv.DictReader(Path(a.audit).open(encoding='utf-8')));reg=json.loads(Path(a.registry).read_text());songs={x['md5'].lower():x for x in reg['songs']};yf=int(reg['success_rule']['youtube_min_views']);sf=int(reg['success_rule']['spotify_min_streams']);tf=int(reg['success_rule'].get('tiktok_min_videos',100000));rows=[]
 for r in audit:
  ...
  tv=meta.get('tiktok_videos_observed');social='PENDING' if tv is None else bool(int(tv)>=tf)
  ...
  rows.append({**r,'genre':meta['genre'],'style':meta['style'],'cohort_key':cohort,'identity_score':identity_score,'identity_high_confidence':identity_high,'duration_delta_seconds':delta,'youtube_views_observed':meta['youtube_views_observed'],'youtube_reach_gate':yg,'spotify_streams_observed':meta['spotify_streams_observed'],'spotify_reach_gate':sg,'crossplatform_streaming_gate':crossplatform,'tiktok_videos_observed':tv if tv is not None else '','tiktok_reach_gate':social,'style_source':meta['style_source'],'youtube_source':meta['youtube_source'],'spotify_source':meta['spotify_source'],'tiktok_source':meta.get('tiktok_source',''),'target_candidate_status':status,'social_network_reach_gate':social,'melodic_reference_gate':'PENDING','scientific_promotion':'PENDING'})
 ...
 cand=[r for r in rows if r['target_candidate_status']=='TARGET_COHORT_CANDIDATE'];social_pass=sum(r['social_network_reach_gate'] is True for r in cand);social_resolved=sum(r['social_network_reach_gate']!='PENDING' for r in cand)
 summary={'schema':'HOOKLAB_TARGET_SCIENTIFIC_CANDIDATE_MATRIX_v1.2','cohort_key':reg['cohort_key'],'rows':len(rows),'target_cohort_candidates':len(cand),'youtube_floor':yf,'spotify_floor':sf,'tiktok_video_floor':tf,'social_reach_pass_candidates':social_pass,'social_reach_resolved_candidates':social_resolved,'remaining_gates':['social_network_reach','melodic_reference_validation'],'scientific_promotion':False}
