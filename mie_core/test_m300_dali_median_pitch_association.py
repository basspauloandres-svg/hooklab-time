...
def main():
 with tempfile.TemporaryDirectory() as td:
  ...
  candidates=[];ident=[]
  for i in range(30):
   title=f'Song {i}';artist=f'Artist {i}';did=f'D{i}';cid=f'M300::X::{i}'
   candidates.append({'candidate_id':cid,'chart_year':2020,'rank':i%15+1,'title':title,'artist':artist,'spotify_playcount_observed':10**(6+i/10)})
   write(ev/f'{i}.json',{'schema':'HOOKLAB_DALI_ANNOTATION_EVIDENCE_v1.0','status':'PASS_ANNOTATION_PARSE','candidate_id':cid,'dali_id':did,'title':title,'artist':artist,'representation_origin':'DALI_NOTE_EVENTS','quality_tier':'HIGH_NCC','ncc':.9,'melody_summary':{'pitch_median_midi':50+i,'n_note_events':100}})
  ...
  write(root/'allow.json',{'allowed_for_population_association_testing':['median_pitch_st']})
  ...
  print('PASS: association runner is representation-allowlisted, identity-gated and never auto-promotes synthetic evidence')
